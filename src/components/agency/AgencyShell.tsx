...
export const AgencyShell = () => {
  ...
  // Full-screen mobile pages: hide top header + bottom nav
  const fullScreenMobile = isMobile && location.pathname.startsWith("/agency/messages");

  return (
    <div className="relative flex min-h-screen w-full bg-agency-canvas">
      <Sidebar collapsed={collapsed} onToggle={() => setCollapsed((v) => !v)} />
      <div className="flex min-w-0 flex-1 flex-col">
        {!fullScreenMobile && <Header />}

        <main className={fullScreenMobile ? "flex-1" : "flex-1 px-3 pb-20 pt-2 lg:px-6 lg:pb-8"}>
          {!fullScreenMobile && (
          ...
          <div className={fullScreenMobile ? "" : "mt-2"}>
            <Outlet />
          </div>
        </main>
      </div>
      {!fullScreenMobile && <MobileNav open={moreOpen} onOpenChange={setMoreOpen} />}
    </div>
  );
};
