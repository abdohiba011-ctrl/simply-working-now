...
const MotorbikeWizard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (editing && id) {
        const { data: bt } = await supabase.from("bike_types").select("*").eq("id", id).maybeSingle();
        if (bt) {
          const b: any = bt;
  ...
  // Auto-save the current step when moving forward
  ...
  const stepErrors = (idx: number): string[] => {
    ...
    if (idx === 0) {
      ...
      // step 3 has no hard required fields
    } else if (idx === 3) {
  ...
  const saveAndExit = async () => {
    if (!bikeId) { navigate("/agency/motorbikes"); return; }
    setSaving(true);
    await persistDraft();
    setSaving(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isDraftFirstSubmit) {
        ...
        // ensure a physical bike row exists
  ...
  return (
    <AgencyLayout>
      <div className="mx-auto max-w-3xl space-y-4 pb-32">
        ...
        {stepIdx === 4 && (
          <Card className="space-y-5 p-6">
            <h2 className="text-lg font-semibold">Pricing & policies</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
        ...
        {/* Sticky bottom nav */}
        <div className="fixed inset-x-0 bottom-0 border-t border-border bg-background/95 backdrop-blur p-3 z-30">
